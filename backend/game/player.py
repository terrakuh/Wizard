...
class Player:
    ...
    def call_tricks(self, called_tricks: int, max_tricks: int) -> str:
        self.logging.debug("Calling call_tricks for " + self.name)
    ...
    def play_card(self, lead_color: str) -> str:
        ...
        def payload(user_input):
            ...
            if card_variants:
                ...
                self.logging.info(self.name + " has selected " + str(variant_selected))
    ...
    class UserTask:
        ...
        def __init__(self, task_type, player, options):
            ...
            self.wait_for_input = self.threading.Event()

            print("Task created")

        def do_task(self, process_input=None):
            self.logging.info(self.player.name + " is waiting for input on " + self.task_type)
            ...
            self.logging.info(self.player.name + "'s input for " + self.task_type + ": " + str(user_input))
